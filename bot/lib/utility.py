...
def log(text:any):
    """LOG funcion"""
    print(text)
    with open('log.json','r') as rfile:
        json_=json.load(rfile)
        json_["LOGS"].append(
            {'time':time.time(),"log-text":f'{text}'}
        )
    rfile.close()
    with open('log.json','w') as wfile:json.dump(obj=json_,fp=wfile,indent=4)
    wfile.close()
def error(errorText:any):
