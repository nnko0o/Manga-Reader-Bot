...
import httpx , aiohttp , aiofiles,os, asyncio
...
async def downloadi_image(url:str,path:str,name:str):
...
async def download_images(images:List,path:str):
	for image in images: await downloadi_image(image,path,image.split("/")[-1])
...
class Manga3asq:
    ...
    async def searchManga(self,searchQeury:str):
        ...
        _SearchData = r.json() 
        if _SearchData["success"]==False:
            try:
                return([
        		f"{_SearchData['data'][0]['error']}"
        	])
            except:return("ERROR")
...
async def getMangaDetiles(MangaURL:str) -> dict:
        ###async with httpx.AsyncClient() as requests:
    res = httpx.get(MangaURL)
    soup = BeautifulSoup(
    res.content,
	"html.parser"
	)
    MangaTitle = soup.findAll("div",{"class","post-title"})[0].text
    MangaCoverImage = soup.findAll("div",{"class":"summary_image" })[0].a.img["src"]
    MangaAuther = soup.find("div",{"class":"author-content"})
    MangaArtist   = soup.find( "div", {"class":"artist-content"} )
    if MangaAuther==None: MangaAuther="غير معرف"
    else:MangaAuther=MangaAuther.text.strip()
    if MangaArtist==None: MangaArtist="غير معرف"
    else: MangaArtist=MangaArtist.text.strip()
    MangaStory   = soup.find("div",{"class":"manga-excerpt"})
    if MangaStory==None: MangaStory="غير معرف"
    else: MangaStory=MangaStory.text.strip()
    print(MangaTitle,MangaCoverImage,MangaAuther, MangaArtist, MangaStory)
    return(
	{ 
	"title":MangaTitle, 
...
#	asyncio.run(
#	getMangaDetiles(
#			MangaURL="https://3asq.org/manga/one-piece/"
#		)
#	)